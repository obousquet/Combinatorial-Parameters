...
\noindent\textbf{Statement.} The Recursive Teaching Dimension of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the sole concept has an empty teaching set. For $n\ge2$, in any nontrivial remaining subfamily, every singleton is taught by its unique positive coordinate and no empty sample identifies a concept. Remove the concepts in any order, using these positive samples until the last concept, which has an empty teaching set. The maximum size is one, and the first nontrivial stage rules out zero.
\end{proof}
...
\noindent\textbf{Statement.} The Order Sample Compression of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, order only the sole concept and reconstruct it from the empty key. For $n\ge2$, order the all-zero word first, followed by the singleton concepts in any fixed order. An all-negative sample has the zero word as its first consistent output, determined by the empty key. A sample with a positive coordinate has its singleton as its unique consistent word in this order, determined by that labeled positive coordinate. Thus width one suffices. Width zero cannot reconstruct two distinct full concepts.
\end{proof}
...
\begin{proof}
Fix any order of the singleton hypotheses. For the lower bound, take the last hypothesis and the all-negative sample on the other $n-1$ coordinates. Each earlier hypothesis is excluded only by the negative observation at its own coordinate. Omitting any of these observations makes an earlier hypothesis consistent, so all $n-1$ are necessary to preserve the first consistent output. For the upper bound when $n\ge2$, a sample with a positive observation retains that observation. For an all-negative sample, let $h_r$ be its first consistent singleton in the fixed order; retain the negative coordinates of the preceding $r-1$ hypotheses. Those observations belong to the sample and determine $h_r$ as its first consistent output. The largest key size is $n-1$. For $n=1$, the empty key reconstructs the sole concept.
\end{proof}
\noindent\textbf{Reference.} \cite{darnstadt2016order}.
...
\noindent\textbf{Statement.} The Partial Equivalence Queries Complexity of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the target is known without a query. For $n\ge2$, query the all-zero labeling. Every singleton target disagrees with it only at its unique positive coordinate, so any counterexample identifies the target. Zero queries cannot distinguish the $n$ possible targets. A total labeling is an allowed partial equivalence query.
\end{proof}
...
\noindent\textbf{Statement.} The Membership and Equivalence Queries Complexity of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the target is known without a query. For $n\ge2$, query the all-zero labeling. Every singleton target disagrees with it only at its unique positive coordinate, so any counterexample identifies the target. Zero queries cannot distinguish the $n$ possible targets. Arbitrary equivalence queries are allowed in the mixed model; adding membership queries cannot improve on zero or one.
\end{proof}

\subsection{Minimum Teaching Set Size of Singletons}\label{val:minimum_teaching_set_size_singletons}
\noindent\textbf{Statement.} The Minimum Teaching Set Size of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the empty teaching sample identifies the sole concept. For $n\ge2$, the positive label at the unique point of a singleton identifies it, while the empty sample is consistent with all $n$ concepts. Thus every concept has minimum teaching-set size one, even when teaching samples must be positive.
\end{proof}
...
\noindent\textbf{Statement.} The Projected Recursive Teaching Dimension of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, every projection has one trace and recursive teaching dimension zero. For $n\ge2$, every projection consists of singleton traces, possibly together with the empty trace. Remove singleton traces first, using their unique positive coordinates whenever competitors remain; the final trace has an empty teaching set. This is a positive recursive plan of width at most one, hence also an ordinary one. The identity projection is nontrivial, so recursive width zero is impossible.
\end{proof}

\subsection{Teaching Dimension of Singletons}\label{val:teaching_dimension_singletons}
\noindent\textbf{Statement.} The Teaching Dimension of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the empty teaching sample identifies the sole concept. For $n\ge2$, the positive label at the unique point of a singleton identifies it, while the empty sample is consistent with all $n$ concepts. Thus every concept has minimum teaching-set size one, even when teaching samples must be positive.
\end{proof}
...
\noindent\textbf{Statement.} The Projected Positive Recursive Teaching Dimension of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, every projection has one trace and recursive teaching dimension zero. For $n\ge2$, every projection consists of singleton traces, possibly together with the empty trace. Remove singleton traces first, using their unique positive coordinates whenever competitors remain; the final trace has an empty teaching set. This is a positive recursive plan of width at most one, hence also an ordinary one. The identity projection is nontrivial, so recursive width zero is impossible.
\end{proof}
...
\noindent\textbf{Statement.} The Projected No-Clashing Teaching Dimension of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, every projection has one trace, taught by the empty sample. For $n\ge2$, teach each singleton trace by its positive coordinate and the possible empty trace by the empty sample. For two distinct traces, at least one is a singleton whose positive coordinate distinguishes it from the other, so the map is positive and no-clashing. Its width is at most one. On the nontrivial identity projection, width zero would assign the empty sample to every target and make two distinct targets clash. Thus both ordinary and positive projected no-clashing dimensions equal one.
\end{proof}
...
\noindent\textbf{Statement.} The Projected Positive No-Clashing Teaching Dimension of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, every projection has one trace, taught by the empty sample. For $n\ge2$, teach each singleton trace by its positive coordinate and the possible empty trace by the empty sample. For two distinct traces, at least one is a singleton whose positive coordinate distinguishes it from the other, so the map is positive and no-clashing. Its width is at most one. On the nontrivial identity projection, width zero would assign the empty sample to every target and make two distinct targets clash. Thus both ordinary and positive projected no-clashing dimensions equal one.
\end{proof}
...
\noindent\textbf{Statement.} The Littlestone Dimension of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, one fixed concept cannot realize a complete depth-one binary tree. For $n\ge2$, any coordinate takes both labels, giving depth one. At any candidate root, the positive branch has only one singleton concept and cannot split again. Hence a complete depth-two tree is impossible.
\end{proof}
...
\noindent\textbf{Statement.} The Maximum Teaching Set Size of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the empty teaching sample identifies the sole concept. For $n\ge2$, the positive label at the unique point of a singleton identifies it, while the empty sample is consistent with all $n$ concepts. Thus every concept has minimum teaching-set size one, even when teaching samples must be positive. Taking the maximum of these minimum sizes gives the claimed value.
\end{proof}
...
\begin{proof}
Fix a target singleton in a subfamily of $m$ singleton concepts on the original domain. Project onto the $m-1$ coordinates of its competitors. The target becomes the empty trace, and each competitor becomes a different singleton trace; teaching the target requires all $m-1$ negative coordinates. Conversely, in any projection there are at most $m-1$ other distinct traces. Choose one coordinate of disagreement with each such trace to teach the target using at most $m-1$ observations. Thus every target has worst projected teaching-set size exactly $m-1$, including zero at $m=1$. Applying this to the full class with $m=n$ and minimizing over its targets gives $n-1$.
\end{proof}
...
\noindent\textbf{Statement.} The Equivalence Queries Complexity of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the target is known without a query. For $n\ge2$, query the all-zero labeling. Every singleton target disagrees with it only at its unique positive coordinate, so any counterexample identifies the target. Zero queries cannot distinguish the $n$ possible targets. This is a direct proof and does not require a general comparison with Littlestone dimension.
\end{proof}
\noindent\textbf{Reference.} \cite{littlestone1988learning}.
...
\noindent\textbf{Statement.} The Positive Teaching Dimension of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the empty teaching sample identifies the sole concept. For $n\ge2$, the positive label at the unique point of a singleton identifies it, while the empty sample is consistent with all $n$ concepts. Thus every concept has minimum teaching-set size one, even when teaching samples must be positive.
\end{proof}
\noindent\textbf{Reference.} \cite{balbach2008measuring}.
...
\noindent\textbf{Statement.} The Positive Recursive Teaching Dimension of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the sole concept has an empty teaching set. For $n\ge2$, in any nontrivial remaining subfamily, every singleton is taught by its unique positive coordinate and no empty sample identifies a concept. Remove the concepts in any order, using these positive samples until the last concept, which has an empty teaching set. The maximum size is one, and the first nontrivial stage rules out zero.
\end{proof}
\noindent\textbf{Reference.} \cite{fallat2023batch}.
...
\noindent\textbf{Statement.} The Stable Unlabeled Complexity of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, always reconstruct the sole concept from the empty key. For $n\ge2$, decode the empty key as the all-zero word and the coordinate key $\{i\}$ as singleton $\{i\}$. A realizable sample with a positive coordinate retains that coordinate; an all-negative sample retains nothing. This uses no retained label or side information. Every intermediate subsample containing the retained key still has that positive observation, or is again all-negative when the key is empty. Hence the key and reconstruction are stable. Two distinct full concepts forbid width zero. The empty-key output is improper for $n\ge2$.
\end{proof}
...
\noindent\textbf{Statement.} The Stable Labeled Sample Compression of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
Use the stable unlabeled decoder: at $n=1$, always output the sole concept from the empty key. For $n\ge2$, retain the unique positive observation when present and otherwise retain nothing; output that singleton or the all-zero word, respectively. The decoder may ignore the retained label. Every intermediate sample containing the key makes the same choice, so the scheme is stable. Two distinct full concepts rule out width zero. Properness is not asserted.
\end{proof}
...
\noindent\textbf{Statement.} The Worst Mistakes of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the known target can be predicted without error. For $n\ge2$, predict zero until the first positive label appears; that label identifies the target singleton, after which all predictions are correct. Thus at most one mistake occurs, even if coordinates repeat. For the lower bound, choose a sequence consisting of one coordinate. Both labels occur among the possible singleton targets, so any prediction can be falsified by a target consistent with that label. Knowledge of the entire presented sequence cannot avoid this first mistake.
\end{proof}
\noindent\textbf{Reference.} \cite{ben1997online}.
...
\begin{proof}
Fix a target singleton in a subfamily of $m$ singleton concepts on the original domain. Project onto the $m-1$ coordinates of its competitors. The target becomes the empty trace, and each competitor becomes a different singleton trace; teaching the target requires all $m-1$ negative coordinates. Conversely, in any projection there are at most $m-1$ other distinct traces. Choose one coordinate of disagreement with each such trace to teach the target using at most $m-1$ observations. Thus every target has worst projected teaching-set size exactly $m-1$, including zero at $m=1$. Applying this to the full class with $m=n$ and maximizing over its targets gives $n-1$.
\end{proof}
\noindent\textbf{Reference.} \cite{hanneke2015minimax}.
...
\begin{proof}
Fix a target singleton in a subfamily of $m$ singleton concepts on the original domain. Project onto the $m-1$ coordinates of its competitors. The target becomes the empty trace, and each competitor becomes a different singleton trace; teaching the target requires all $m-1$ negative coordinates. Conversely, in any projection there are at most $m-1$ other distinct traces. Choose one coordinate of disagreement with each such trace to teach the target using at most $m-1$ observations. Thus every target has worst projected teaching-set size exactly $m-1$, including zero at $m=1$. Every nonempty subfamily has $m\le n$ and minimum projected teaching size $m-1$. The full class attains $n-1$, so the maximum over subfamilies is exactly $n-1$.
\end{proof}
...
\noindent\textbf{Statement.} The Average Teaching Dimension of Singletons is $0$ for $n=1$; $1$ for $n\ge2$.
\begin{proof}
For $n=1$, the empty teaching sample identifies the sole concept. For $n\ge2$, the positive label at the unique point of a singleton identifies it, while the empty sample is consistent with all $n$ concepts. Thus every concept has minimum teaching-set size one, even when teaching samples must be positive. Averaging these equal individual minimum sizes gives the claimed value.
\end{proof}
