...
\begin{proof}
Every coordinate trace of the n-singleton class consists of distinct singleton concepts, possibly together with the empty concept. Each singleton has at most one smaller one-coordinate neighbour, namely the empty trace. On any two-coordinate trace that empty trace and two singleton traces occur, so the maximum positive degree is exactly one.
\end{proof}
...
\noindent\textbf{Statement.} The Proper Unlabeled Sample Compression of Singletons is $2$.
\begin{proof}
